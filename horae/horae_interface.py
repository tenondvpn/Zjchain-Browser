...
class HoraeInterface(tools_util.Singleton):
            ...
            if not hasattr(self, "a"):
                if logger is not None:
                    ...
                    horae_logger = common_logger.get_logger(
                            "common", 
                            "./log/horae")
                self.__zk_manager = zk_manager.ZookeeperManager(hosts=settings.ZK_HOSTS, logger=horae_logger)
                self.__pipeline_mgr = pipeline_manager.PipelineManager(
                        horae_logger, self.__zk_manager)
    ...
    def update_pipeline(
        ...
        Args:
            ...
            monitor_way: int 报警方式0邮件，1短信，2二者都报
    ...
    def get_server_tags(self, user_id):
        return self.__pipeline_mgr.get_server_tags(user_id)
    ...
    def create_new_pipeline(
        ...
        Args:
            ...
            owner_id：int 创建者id
            manager_id_list: int 拥有管理权限的用户id列表
            monitor_way: int 报警方式 0邮件，1短信，2二者都报
            tag: string 标签, 多个标签','分隔
            description: string 描述
    ...
    def search_processor(self, user_id, word, with_project=1, limit=100):
        """
            ...
            Args:
                user_id: int DAG流原始owner
                word: string 搜索关键词
                with_project: int 0只搜索插件， 1 附带插件的所有分类信息并展开
                limit: int 搜索条数
    ...
    def show_run_history(
        ...
        """
        查看任务
        从horae_runhistory中获取执行历史信息，支持排序，翻页，搜索功能

        Args:
            show_tag: int 0 展示任务历史 1 展示DAG流历史
            owner_id: int 用户id
            page_min: int 翻页最小索引
            page_max：int 翻页最大索引
    ...
    def run_tasks(self, owner_id, task_id_list, run_time_str, ordered=0):
        ...
        Args:
            ...
            run_time_str: string run_time的组合，
                ...
                支持天级别，小时级别，分钟级别
            ordered: 用户提交串行执行 <= 0, 默认执行方式，>0 串行并发orderedDAG流执行
    ...
    def upload_package_with_local(self, user_id, proc_id, version_name,
                                  desc, git_url=None, file_name=None):
        """
            搜索DAG流信息
            ...
            Args:
                user_id: int DAG流原始owner
                proc_id: int 插件id
                version_name： string 插件版本名
                desc： string 版本描述
                git_url： string 如果不为none，则通过git创建插件版本
                file_name： string 如果不为None，则通过本地文件创建插件版本
        ...
        ret = self.__processor_mgr.upload_package_with_local(
            user_id, proc_id, version_name, desc, git_url, file_name)
        use_time = time.time() - beign_time
    ...
    def upload_pacakge(
        ...
        Args:
            processor_id: int 插件id
            owner_id: int 用户id
            file_path： string 插件包路径
    ...
    def create_online_package(self, owner_id, pipeline_id):
        beign_time = time.time()
        ret = self.__pipeline_mgr.create_online_package(owner_id, pipeline_id)
        use_time = time.time() - beign_time
        self.__log.info("create_online_package "
                 "pipeline_id: %s, owner_id: %s"
                 "[use_time:%s]"
                 % (pipeline_id, owner_id, use_time))
        return ret