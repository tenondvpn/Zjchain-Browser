...
class ProcessorManager(object):
    ...
    def __init__(self, logger, zk_mgr):
        ...
        self.__sql_manager = tools_sql_manager.SqlManager(logger, zk_mgr)
    ...
    def get_git_code(self, proc_id, git_url):
        ...
        file_path = os.path.join(self.__work_package_dir, str(proc_id), "git")
    ...
    def upload_package_with_local(self, user_id, proc_id, version_name, desc, git_url, file_name):
        '''
        git clone  https://actantion:qincai521@github.com/EigenLab/user_project.git
        '''
        ...
        if len(his_uploads) > 0:
            ...
            upload_his.save()
            t = time.time()
            sql = ("update horae_task set last_update_time_us=%d where version_id=%d;" % (
                int(round(t * 1000000)), upload_his.id))
            cursor = django.db.connection.cursor()
            cursor.execute(sql)
            tasks = horae.models.Task.objects.filter(version_id=upload_his.id)
            proc_info = horae.models.Processor.objects.get(id=upload_his.processor_id);
            for task in tasks:
                ...
                task_json = {
                    ...
                    "plugin_id": task.pid,
                    "params": params,
                    "plugin_version": upload_his.version,
                    "type": proc_info.type,
                ...
                if self.__zk_manager.exists(task_path):
                    ...
                    if res is None:
                        raise Exception("修改去中心化配置失败！ " + task_path)
        else:
        ...
        save_id = "%s-%s" % (proc_id, version_name)
        ...
        self.__log.error("upload work pacakge success! %s %s/%s.tar.gz" % (file_name, self.__work_package_dir, save_id))
        ...
        for key in range(-1, -4, -1):
            ...
            for processor in processors:
                tmp_map[processor.project_id].append({
                    ...
                    "text": processor.name, "is_project": 0, "iconCls": "icon-filter"})
    ...
    def delete_proc_version(self, user_id, proc_id, id):
        self.__shell_cmd.run_once("rm -rf %s/%s-%s.tar.gz" % (self.__work_package_dir, proc_id, id))
        '''
    ...
    def upload_pacakge(
        ...
        if len(upload_histories) > 0:
            ...
            t = time.time()
            sql = ("update horae_task set last_update_time_us=%d where version_id=%d;" % (
                int(round(t * 1000000)), upload_history.id))
            cursor = django.db.connection.cursor()
            cursor.execute(sql)
            tasks = horae.models.Task.objects.filter(version_id=upload_history.id)
            proc_info = horae.models.Processor.objects.get(id=upload_history.processor_id);
            for task in tasks:
                params_split = task.config.split("\r\n");
                params = {}
                for param in params_split:
                ...
                task_json = {
                    ...
                    "plugin_id": task.pid,
                    "params": params,
                    "plugin_version": upload_history.version,
                    "type": proc_info.type,
                    "last_update_time_us": task.last_update_time_us,
                }
    ...
    def __handle_genaral_package(
            ...
            file_path,
            overwrite):
        save_id = "%s-%s" % (processor_id, version_name)
        self.__shell_cmd.run_once("rm -rf %s/%s.tar.gz" % (self.__work_package_dir, save_id))
        self.__shell_cmd.run_once("mv %s/%s.tar.gz %s/%s.tar.gz" % (file_path, processor_id, self.__work_package_dir, save_id))
        self.__log.info("upload work pacakge success!, src[%s/%s.tar.gz], des %s/%s.tar.gz" % (
            file_path, processor_id, self.__work_package_dir, save_id))
        '''
        file_object = self.__oss_package_dir + "/" + save_id + ".tar.gz"
