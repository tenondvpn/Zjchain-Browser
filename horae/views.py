...
def link_task(request):
    if request.method == 'POST':
        ...
        try:
            ...
            rcm_context = request.POST.get('rcm_context')
            rcm_topic = request.POST.get('rcm_topic')
...
def update(request, pipe_id):
    ...
    return render(request, 'update_pipeline.html',
                  {'form': form, 'pipeline': pipeline_info, 'is_super': is_super, 'page_title': '修改DAG流',
...
def update_task(request, task_id):
    ...
    if request.method == 'POST':
        ...
        history_list = get_processor_upload_history(int(task_info['pid']))
...
def update_edge(request, edge_id):
    user = request.user
    is_super = is_admin(user)
    task = Task.objects.get(id=edge_id)
    if request.method == 'POST':
        form = TaskForm(request.POST, instance=task)
        if form.is_valid():
            try:
...
def get_server_tags(request):
    if request.method == 'POST':
        ...
        user = request.user
        try:
            server_tags = horae_interface.get_server_tags(user.id)
            status = 0
            msg = 'OK'
        except Exception as ex:
...
def create_task_choose_proc(request):
    ...
    return render(request, 'create_task_choose_proc.html',
                  {'processor': [],
                   ...
                   'owner_name': '', 'page_title': '插件详情', 'pipeline_model': 1,
                   'page_index': 4
                   })
...
def get_shared_processor_tree_async(request):
    ...
    if tree_id == 0:
        return JsonHttpResponse([{"is_project": 1, "text": "我创建的插件", "id": -1, "state": "closed"},
                {"is_project": 1, "text": "共享给我的插件", "id": -2, "state": "closed"},
                {"is_project": 1, "text": "标准库插件", "id": -3, "state": "closed"}])
...
def create_edge(request):
    ...
    if request.method == 'POST':
        return JsonHttpResponse(
            {'status': status, 'msg': "invalid post"})
    else:
...
def create(request):
    ...
    return render(request, 'create_pipeline.html',
                  {'form': form, 'page_title': '创建DAG流', 'pipeline_model': 1, 'page_index': 2, 'is_super': is_super})
...
def update_processor(request, processor_id):
    ...
    return render(request, 'update_processor.html',
                  {'form': form, 'processor': processor_info,
                   ...
                   'page_title': '修改插件', 'pipeline_model': 1,
...
def history(request):
    ...
    if status == "success":
        return render(request, 'pipeline_history.html',
                      {'is_super': is_super, 'page_title': '执行历史', 'user': user, 'pipename': '', 'status': '2',
                       'runtime': runtime, 'pipeline_model': 1, 'page_index': 3})
    elif status == "fail":
        return render(request, 'pipeline_history.html',
                      {'is_super': is_super, 'page_title': '执行历史', 'user': user, 'pipename': '', 'status': '3',
                       'runtime': runtime, 'pipeline_model': 1, 'page_index': 3})
    elif pl_name:
        return render(request, 'pipeline_history.html',
                      {'is_super': is_super, 'page_title': '执行历史', 'user': user, 'pipename': pl_name,
                       'taskname': task_name, 'status': '', 'runtime': runtime, 'pipeline_model': 1, 'page_index': 3})
    else:
        return render(request, 'pipeline_history.html',
                      {'is_super': is_super, 'page_title': '执行历史', 'user': user, 'pipename': '', 'status': '',
...
def list_history_condition(request):
   ...
   start_time = request.POST.get('columns[0][search][value]');             #开始时间
    ...
    pipeline_name = request.POST.get('pipeline_name')   #DAG流名
...
def historypipe(request, pipe_id):
    ...
    return render(request, 'pipeline_history.html',
                  {'is_super': is_super, 'page_title': '执行历史',
...
def upload_package_with_local(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if type == 0:  # git
            ...
            elif type == 1:  # local file
                file_obj = request.FILES.get('file')
                if not file_obj.name.strip().endswith(".zip"):
                    raise Exception("本地文件上传，必须是zip文件。")

                file_path = os.path.join(settings.PACKAGE_PATH, str(proc_id))
                ...
                raise Exception("不支持的上传插件方式[%s]" % type)

            res = horae_interface.upload_package_with_local(
                user.id, proc_id, version_name, desc, git_url=git_url, file_name=file_name)
            return JsonHttpResponse(res)
        except Exception as ex:
...
def get_status_graph(request):
    ...
    return render(request, 'status_graph.html',
                  {'is_super': is_super, 'page_title': '任务执行历史', 'user': user, 'pipelineid': pl_id,
...
@login_required(login_url='/login/')
def download_online(request, pipe_id):
    user = request.user
    status, file_path = horae_interface.create_online_package(user.id, pipe_id);
    if status != 0:
        return "error!"

    file=open(file_path,'rb')
    down_load_name = "online_" + str(pipe_id) + ".tar.gz"
...
def download_dev(request):
    file=open("/data/xl/tools/Dags-Dashbord/tools/develop.tar.gz", 'rb')
    down_load_name = "develop.tar.gz"
    response = StreamingHttpResponse(file)
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="{0}"'.format(down_load_name)
    return response
...
def download_python(request):
    file=open("/data/xl/tools/Dags-Dashbord/tools/python3.10.tar.gz", 'rb')
    down_load_name = "python3.10.tar.gz"
    response = StreamingHttpResponse(file)
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="{0}"'.format(down_load_name)
    return response
...
def dags(request):
    file=open("/data/xl/tools/Dags-Dashbord/tools/dags.sh", 'rb')
    down_load_name = "dags.sh"
    response = StreamingHttpResponse(file)
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="{0}"'.format(down_load_name)
    return response
...
def upload_processor(request):
    if request.method == 'POST':
        ...
        if proc_name is None or proc_name.strip() == "":
            return JsonHttpResponse({'status': 1, 'msg': '没有填写插件名.'})
        else:
            processor = Processor.objects.filter(name=proc_name)
            if len(processor) == 0:
                return JsonHttpResponse({'status': 1, 'msg': '插件名不存在.'})
