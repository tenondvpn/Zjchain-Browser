...
function hist_table(list) {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        content += '</td><td>' +
            '<button type="button" class="btn btn-info pull-right"  onclick="create_task_now(' + proc_id + ',' + list[i].id + ',' + $("#proc_type").val() + ',\'' + $("#proc_name").val() + '\',\'' + list[i].name + '\')" title="选择这个插件创建任务。" style="margin-right: 1px;">创建任务</button>' +
            '</td></tr>';
    }
    return content;
}
        ...
        success: function (result) {
            if (result.status == 0) {
                ...
                if (result.processor.private == 1 && result.is_owner == 1) {
                    publish_str = '<a title=\'发布\' onclick=\'publishNode(' + result.processor.id + ', "发布后所有人都可以使用这个插件，确定开放此插件吗?")\' href=\'javascript:void(0)\'><i class=\'fa fa-share\'></i></a>';
                }

                if (result.processor.private != 1 && result.is_owner == 1) {
                    publish_str = '<a title=\'收回\' onclick=\'publishNode(' + result.processor.id + ', "确定收回此插件的公共权限吗?")\' href=\'javascript:void(0)\'><i class=\'fa fa-share\'></i></a>';
                }
