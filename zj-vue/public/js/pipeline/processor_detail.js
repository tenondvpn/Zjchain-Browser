        ...
        success: function (result) {
            if (result.status == 0) {
                ...
                if (result.processor.private == 1 && result.is_owner == 1) {
                    publish_str = '<a title=\'发布\' onclick=\'publish_node(' + result.processor.id + ', "发布后所有人都可以使用这个插件，确定开放此插件吗?")\' href=\'javascript:void(0)\'><i class=\'fa fa-share\'></i></a>';
                }

                if (result.processor.private != 1 && result.is_owner == 1) {
                    publish_str = '<a title=\'收回\' onclick=\'publish_node(' + result.processor.id + ', "确定收回此插件的公共权限吗?")\' href=\'javascript:void(0)\'><i class=\'fa fa-share\'></i></a>';
                }
                ...
                $("#id_proc_detail_detail_info").html('<table class="items table  table-bordered " style="margin-top:-77px;border:1px solid rgb(219, 219, 219);table-layout:fixed;word-break:break-all;word-wrap:break-all;" cellspacing="1" cellpadding="2" border="1">' +
                    ...
                    '<th style="margin-top:20px;margin-left:120px;border=2px solid rgb(205,205,205);background-color: rgb(242,242,242);width:150px;text-align:center;">ID</th><td style="width:250px;">' + result.processor.id + '</td></tr>' +
    ...
    if (project_id == old_project_id) {
        ...
        if (new_project_node) {
            t.tree("append", {
                ...
                data: [{id: project_id + "-" + pid, text: name, is_project: 0, iconCls: "icon-filter"}]
            });
